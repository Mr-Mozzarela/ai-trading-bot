...
# Значения по умолчанию, чтобы скрипт запускался без переменных окружения
...
def generate_prompt(symbol, price, volume, pnl, min_qty, tick_size, balance_usdt):
    return f"""
Ты — AI-советник для криптовалютного грид-бота. Твоя цель — приумножить капитал трейдера, используя сеточную торговлю.

Пара: {symbol}
Цена: {price} USDT
Объём: {volume}
PnL сессии: {pnl} USDT
Текущий баланс: {balance_usdt} USDT
...
Проанализируй текущую рыночную ситуацию, сравни с предыдущей сеткой, учти затраты на перестройку и оцени потенциальную прибыль.
